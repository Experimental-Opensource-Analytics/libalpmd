...
int SYMEXPORT alpm_add_pkg(alpm_handle_t* handle, alpm_pkg_t* pkg)
{
	...
	if((local = _alpm_db_get_pkgfromcache(handle.db_local, pkgname))) {
		...
		const(char)* localpkgver = local.version_;
...
private int perform_extraction(alpm_handle_t* handle, archive* archive, archive_entry* entry, const(char)* filename)
{
	int ret = void;
...
private int try_rename(alpm_handle_t* handle, const(char)* src, const(char)* dest)
{
	if(rename(src, dest)) {
...
private int extract_db_file(alpm_handle_t* handle, archive* archive, archive_entry* entry, alpm_pkg_t* newpkg, const(char)* entryname)
{
...
private int extract_single_file(alpm_handle_t* handle, archive* archive, archive_entry* entry, alpm_pkg_t* newpkg, alpm_pkg_t* oldpkg)
{
...
version (none) {
	...
	if(notouch || needbackup) {
		if(filename_len + strlen(".pacnew") >= PATH_MAX) {
			_alpm_log(handle, ALPM_LOG_ERROR,
					_("unable to extract %s.pacnew: path too long"), filename.ptr);
			return 1;
		}
...
private int commit_single_pkg(alpm_handle_t* handle, alpm_pkg_t* newpkg, size_t pkg_current, size_t pkg_count)
{
	int ret = 0, errors = 0;
	int is_upgrade = 0;
